...
public:
  ...
  {
    static_assert(std::is_base_of<PoolElement, T>::value || std::is_fundamental<T>::value,
                  "Object need to be either trivial or poolable");
  ...
  ~Pool() { assert(m_all.size() == m_pool.size() && "Some elements are still in use"); }

  bool Empty() { return m_pool.empty(); }

  size_t Count() { return m_pool.size(); }
