...
public:
  ...
  basic_istream(streams::streambuf<CharType> buffer)
    : m_helper(std::make_shared<details::basic_istream_helper<CharType>>(buffer))
  {
  ...
  pplx::task<size_t> read_line(streams::streambuf<CharType> target) const
  {
    ...
    typename concurrency::streams::char_traits<CharType>::int_type req_async =
      concurrency::streams::char_traits<CharType>::requires_async();
...
pplx::task<ReturnType> concurrency::streams::_type_parser_base<CharType>::_parse_input(
  concurrency::streams::streambuf<CharType> buffer, AcceptFunctor accept_character, ExtractFunctor extract)
{
...
public:
  ...
  {
    return concurrency::streams::_type_parser_base<CharType>::template _parse_input<std::basic_string<CharType>,
                                                                                    std::string>(
...
public:
  ...
  {
    return _type_parser_base<CharType>::template _parse_input<_int64_state, int64_t>(
      buffer, _accept_char, _extract_result);
  }
...
public:
  ...
  {
    return _type_parser_base<CharType>::template _parse_input<_double_state<double>, double>(
...
public:
  ...
  {
    return _type_parser_base<CharType>::template _parse_input<_double_state<float>, float>(
...
public:
  ...
  {
    return _type_parser_base<CharType>::template _parse_input<_uint64_state, uint64_t>(
      buffer, _accept_char, _extract_result);
  }
...
public:
  ...
  {
    return _type_parser_base<CharType>::_skip_whitespace(buffer).then([=](pplx::task<void> op) -> pplx::task<char> {
      op.wait();
...
private:
  ...
  {
    concurrency::streams::streambuf<CharType> buf = buffer;
    return buf.bumpc().then([=](pplx::task<typename concurrency::streams::streambuf<CharType>::int_type> op) -> char {
      int_type val = op.get();
      if(val == concurrency::streams::char_traits<CharType>::eof())
        throw std::runtime_error("reached end-of-stream while constructing a value");
      return char(val);
